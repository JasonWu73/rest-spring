...
/**
 * 默认 {@code @PropertySource} 只能读取 {@code .properties} 文件, 故需自己实现对 YAML 文件的读取
 *
 * @author 吴仙杰
...
public class YamlPropertySourceFactory implements PropertySourceFactory {
  ...
  public PropertySource<?> createPropertySource(final String name, final EncodedResource encodedResource) {
    ...
    // 使用 `YamlPropertiesFactoryBean` 将 YAML 格式的资源转换为 `java.util.Properties` 对象
    ...
    if (properties == null) {
      throw new InternalServerException("识别 YAML 配置文件的 FactoryBean 无法完成初始化");
    }
