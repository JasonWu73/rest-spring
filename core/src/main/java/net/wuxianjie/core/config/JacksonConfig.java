...
/**
 * 定制 Spring Boot 的 JSON 序列化 (处理发送至客户端的响应) 与反序列化 (处理发送至服务端的请求)
 *
 * <p>Spring Boot 的默认配置如下:</p>
 * <ul>
...
public class JacksonConfig {
   ...
   *
   * @return 定制后的 JSON 序列化 Bean
   */
  @Bean
  public Jackson2ObjectMapperBuilderCustomizer jsonCustomizer() {
    return builder -> {
      // 序列化后的字符串仅包含非 null 属性
  ...
  /**
   * 用于项目中通过 Spring 依赖注入的预配置 JSON 序列化/反序列化工具 Bean
   *
   * <p>注意: 因未使用{@code @Primary}, 故该 Bean 并没有替代默认 {@code ObjectMapper}, 仅仅作为工具 Bean 使用</p>
   *
   * @return 预配置的 JSON 序列化/反序列化工具 Bean
   */
  @Bean
  public ObjectMapper objectMapper() {
    ...
    // === 定制 JSON 序列化/反序列化 ===
    // 属性命名转换仅对 POJO 有效，对 `java.util.Map` 是无效的
    ...
    // === 配置 Java 8 `LocalDate` 与 `LocalDateTime` 的序列化字符串格式 ===
